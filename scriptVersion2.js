...
function setAnswers(){
	...
	while (currentPos < quiz.length){
		...
		for (n=0; n<shuffledQuiz[currentPos].answers.length;n++){
			...
			var add_text = 
			"<li class='list_item'><label class='label--radio'><input type='radio' class='radio' value="+radio_id+" name="+tempTag+"><div id=list_inputText>"+shuffledQuiz[currentPos].answers[n]+"</div></label></li>";
...
for (i=0;i<shuffledQuiz.length;i++){
	shuffleQstArray.push(shuffledQuiz[i].question);
	console.log('Shuffled: '+shuffleQstArray[i]);
}
...
//var selectedName = selected.siblings().text(); //this gives the answer text.
//console.log('\nAns: '+selectedName);
